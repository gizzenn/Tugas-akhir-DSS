...
<body class="d-flex justify-content-center align-items-center py-5">
    <div class="card card-custom border-0 p-4 mx-3">
        <div class="text-center mb-4">
            ...
            <h4 class="fw-bold text-dark m-0">Login Admin</h4>
            <p class="text-muted small">Sistem Pendukung Keputusan RS</p>
        </div>
        
        <!-- Pastikan action mengarah ke route login.admin -->
<form act<form action="{{ route('login.admin') }}" method="POST">
    @csrf
    
    <div class="mb-3">
        <label class="form-label small fw-semibold text-secondary">Alamat Email Admin</label>
        <input type="email" name="email" class="form-control py-2.5" style="border-radius: 10px;" placeholder="admin@rs.com" required>
    </div>
    
    <div class="mb-4">
        <label class="form-label small fw-semibold text-secondary">Password</label>
        <input type="password" name="password" class="form-control py-2.5" style="border-radius: 10px;" placeholder="••••••••" required>
    </div>
    
    <button type="submit" class="btn btn-dark-blue w-100 fw-bold py-2.5 shadow-sm mb-3">
        Masuk sebagai Admin
    </button>
</form>
    </div>
